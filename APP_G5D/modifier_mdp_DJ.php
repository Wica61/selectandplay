...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($motDePasseActuel) || empty($nouveauMotDePasse) || empty($confirmerMotDePasse)) {
        ...
        $stmt = $db->prepare("SELECT mdp FROM DJ WHERE id_DJ = :userID");
        ...
        if ($result && $result["mdp"] === $motDePasseActuel) {
            ...
            if ($nouveauMotDePasse === $confirmerMotDePasse) {
                ...
                $stmt = $db->prepare("UPDATE DJ SET mdp = :nouveauMotDePasse WHERE id_DJ = :userID");
...
<p><a href="profil_DJ_2.php">Retour sur son profil</a></p>
